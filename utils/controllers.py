...
from utils.inputs.controls import Sources, EventTypes, Event, Controls, Mouse, Inputs
            ...
            if self.source_type == Sources.keyboard and (event.type == pygame.KEYDOWN or event.type == pygame.KEYUP):
                if Controls.key_exists(event.key):
                    if event.type == pygame.KEYDOWN:
                        inputs.add(Event(Controls.from_key(event.key).get_code(), EventTypes.DOWN, source=Sources.keyboard))
                    elif event.type == pygame.KEYUP:
                        inputs.add(Event(Controls.from_key(event.key).get_code(), EventTypes.UP, source=Sources.keyboard))
                ...
                for control in Controls:
                    ...
                    if transform is not None:
                        if event.type == pygame.JOYBUTTONDOWN:
                            inputs.add(Event(transform, EventTypes.DOWN, source=Sources.controller))
                        elif event.type == pygame.JOYBUTTONUP:
                            inputs.add(Event(transform, EventTypes.UP, source=Sources.controller))
            # Si la source est une souris
                ...
                if Mouse.key_exists(event.button):
                    if event.type == pygame.MOUSEBUTTONUP:
                        inputs.add(Event(event.button, EventTypes.UP, source=Sources.mouse))
                    elif event.type == pygame.MOUSEBUTTONDOWN:
                        inputs.add(Event(event.button, EventTypes.DOWN, source=Sources.mouse))
        return inputs
