    ...
    def game_update():
        if game.actual_menu is not None and not isinstance(game.actual_menu, GameMenu):
            ...
            for player_ in game.players:
                player_.attack()
                if player_.get_event(pygame.K_z, ControlsEventTypes.DOWN):
                    player_.entity.incline += 1
                if player_.get_event(pygame.K_s, ControlsEventTypes.DOWN):
                    player_.entity.incline -= 1
                if player_.get_event(pygame.K_c, ControlsEventTypes.DOWN):
                    Wall(player)
                if player_.get_event(pygame.K_v, ControlsEventTypes.DOWN):
                    Turret(player)
